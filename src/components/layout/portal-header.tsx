      ...
      <div className="flex items-center gap-4">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="h-12 rounded-2xl px-2 hover:bg-zinc-50 flex items-center gap-3 transition-all border border-transparent hover:border-zinc-100 group">
              <div className="flex flex-col items-end hidden sm:flex">
                <span className="text-xs font-bold text-zinc-900 leading-none group-hover:text-primary transition-colors">{fullName}</span>
          ...
          <DropdownMenuContent align="end" className="w-64 rounded-3xl p-3 border-none shadow-2xl mt-2 animate-in fade-in-0 zoom-in-95">
            ...
            {role === 'admin' && (
              <DropdownMenuItem asChild className="rounded-2xl cursor-pointer py-3 focus:bg-primary/5 focus:text-primary group">
                <Link href="/admin/settings" className="flex items-center w-full font-bold">
                  <Settings className="h-4 w-4 mr-3 text-zinc-400 group-focus:text-primary" /> System Settings
                </Link>
              </DropdownMenuItem>
            )}
            <DropdownMenuItem 
              onClick={handleSignOut}
              className="rounded-2xl cursor-pointer py-3 text-destructive focus:bg-destructive focus:text-white font-bold group"
            >
              <LogOut className="h-4 w-4 mr-3 text-destructive/50 group-focus:text-white" /> Sign Out
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
