...
import { MessageSquare, Send, Clock, CheckCircle, Plus, Loader2, Sparkles, AlertCircle } from 'lucide-react';
  ...
  const handleSubmit = async () => {
    ...
    try {
      await addDoc(collection(firestore, 'inquiries'), {
        ...
        id: '' // Firestore generates ID, but we set it to empty string for our schema if required
      });
          ...
          <div className="space-y-4">
            {isLoading ? (
              ...
              <Card className="border-none shadow-sm rounded-3xl p-12 text-center bg-white">
                ...
                <h3 className="text-xl font-bold font-headline mb-2">No active inquiries</h3>
                <p className="text-muted-foreground mb-8">Need help with academic documents? Click 'New Inquiry' to start a conversation with CICS Admins.</p>
                <Button variant="outline" className="rounded-full px-8" onClick={() => setIsSubmitOpen(true)}>
                  Create my first inquiry
                </Button>
              </Card>
            ) : (
              inquiries?.sort((a,b) => new Date(b.submissionDate).getTime() - new Date(a.submissionDate).getTime()).map((iq) => (
