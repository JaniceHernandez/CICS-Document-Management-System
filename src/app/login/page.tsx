    ...
    async function handleAuthFlow() {
      ...
      try {
        ...
        if (userData?.status === 'blocked') {
          toast({ title: "Account Blocked", description: "Your institutional access has been suspended.", variant: "destructive" });
          await signOut(auth);
        ...
        if (targetRole === 'admin') {
          ...
          const emailId = user.email?.toLowerCase().trim().replace(/[^a-z0-9]/g, '_') || '';
          const authorizedAdminRef = doc(firestore, 'authorizedAdmins', emailId);
          const authorizedSnap = await getDoc(authorizedAdminRef);

          const isAdmin = adminSnap.exists() || authorizedSnap.exists() || user.email === 'admin@neu.edu.ph';
          
          if (!isAdmin) {
            toast({ title: "Unauthorized Access", description: "This account does not have administrative clearance.", variant: "destructive" });
            await signOut(auth);
            setIsAuthenticating(false);
            return;
          }
        ...
        if (isAuthorizedDomain) {
          ...
          toast({ title: "Access Denied", description: "Students must authenticate with their @neu.edu.ph account.", variant: "destructive" });
          await signOut(auth);
          setIsAuthenticating(false);
        }
      } catch (error: any) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error.code !== 'auth/popup-closed-by-user') {
        toast({ title: "Authentication Failed", description: error.message || "Institutional sign-in failed.", variant: "destructive" });
      }
      setIsAuthenticating(false);
    }
  };
      ...
      <div className="w-full max-w-lg space-y-10 relative z-10">
        ...
        <Card className="border-none shadow-[0_32px_64px_-12px_rgba(0,0,0,0.14)] rounded-[2.5rem] overflow-hidden bg-white">
          <CardHeader className="space-y-2 p-10 pb-8 text-center bg-zinc-50/50 border-b relative">
            <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-6 py-1 bg-primary text-white rounded-full text-[9px] font-bold uppercase tracking-widest shadow-xl">Official Entrance</div>
            <CardTitle className="text-3xl font-headline font-bold text-primary flex items-center justify-center gap-3">
              {targetRole === 'admin' ? <><ShieldCheck className="h-7 w-7 text-secondary" /> Admin Portal</> : <><GraduationCap className="h-7 w-7 text-secondary" /> Student Portal</>}
            </CardTitle>
            <CardDescription className="text-base font-medium text-zinc-500">
              {targetRole === 'admin' ? 'Authorized CICS staff and faculty access only.' : 'Please use your NEU institutional credentials.'}
            </CardDescription>
          </CardHeader>
          
          <CardContent className="p-10 space-y-8">
            <div className="space-y-6">
              <Button variant="outline" className="w-full h-16 rounded-[1.25rem] font-bold text-lg border-2 border-zinc-100 hover:bg-zinc-50 flex items-center justify-center gap-4 transition-all hover:scale-[1.02] active:scale-95" onClick={handleGoogleLogin} disabled={isAuthenticating}>
                ...
                <form onSubmit={handleAdminEmailLogin} className="space-y-6">
                  <div className="space-y-3">
                    ...
                    <div className="relative group">
                      ...
                      <Input id="email" type="email" placeholder="admin@neu.edu.ph" value={email} onChange={(e) => setEmail(e.target.value)} className="pl-12 h-14 rounded-2xl bg-zinc-50 border-none focus-visible:ring-2 focus-visible:ring-primary/20 shadow-inner" disabled={isAuthenticating} required />
                    </div>
                  </div>
                  <div className="space-y-3">
                    <Label htmlFor="password" className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-400 ml-1">Password</Label>
                    <div className="relative group">
                      ...
                      <Input id="password" type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className="pl-12 h-14 rounded-2xl bg-zinc-50 border-none focus-visible:ring-2 focus-visible:ring-primary/20 shadow-inner" disabled={isAuthenticating} required />
                    </div>
                  </div>
                  <Button type="submit" className="w-full h-16 rounded-[1.25rem] font-bold text-lg shadow-2xl shadow-primary/20 transition-all hover:scale-[1.02] active:scale-95" disabled={isAuthenticating}>
                    {isAuthenticating ? <Loader2 className="h-6 w-6 animate-spin" /> : 'Log In with Credentials'}
                  </Button>
                </form>
              )}
          ...
          <CardFooter className="bg-zinc-50/50 p-8 flex justify-center border-t">
            <Link href={`/login?role=${targetRole === 'admin' ? 'student' : 'admin'}`} className="text-[11px] font-bold text-primary hover:underline uppercase tracking-[0.2em] flex items-center gap-2" onClick={() => setIsAuthenticating(false)}>
              {targetRole === 'admin' ? <>Switch to Student Access <ArrowLeft className="h-3 w-3 rotate-180" /></> : <>Switch to Admin Access <ArrowLeft className="h-3 w-3 rotate-180" /></>}
            </Link>
          </CardFooter>
        </Card>
        
        <p className="text-center text-zinc-400 text-[9px] font-bold uppercase tracking-widest pb-10">© 2026 New Era University • College of Informatics and Computing Studies</p>
      </div>
    </div>
  );
}
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-zinc-50 flex items-center justify-center"><Loader2 className="h-12 w-12 text-primary animate-spin" /></div>}>
      <LoginContent />
    </Suspense>
  );
}
